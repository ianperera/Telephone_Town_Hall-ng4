...
import {ConferenceSetup, ConferencePin} from "../../models/conference";
...
@Component({
    ...
    templateUrl: './call-participant.tmpl.html',
    styleUrls: ['./call-participant.css']
})
export class CallParticipantComponent implements OnInit {
    ...
    performRoleOperation(): void {
        if (!!this.confData.pins && this.confData.pins.length == 0) {
            this.summaryData = [];
            return;
        }
