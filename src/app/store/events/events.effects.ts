...
import * as fromQuestions from '../questions/questions.actions';
...
function eventMap(event: events.StreamEvents): Action {
  switch (event.type) {
    ...
    case 'sdQuestionUpdate':
      return new fromQuestions.Update(event.question);
    case 'sdQuestionRemoved':
      return new fromQuestions.Remove({id: event.questionId});
...
export class EventsEffects {
  ...
  startPollAggregator$: Observable<Action> = this.actions$
    ...
    .map((a: fromEvents.PollSuccess | fromEvents.PollFailure) => 
      new fromEvents.PollStart({conferenceId: a.payload.conferenceId}));
