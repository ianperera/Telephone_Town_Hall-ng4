import { concat, filter, takeRight, assign } from 'lodash';
...
export function reducer(state = initialState, action: chat.Actions): State {
  switch (action.type) {
    case chat.CHAT_ALERT:
      return {...state, alerts: concat(state.alerts, assign({}, action.payload))};
    ...
    case chat.CHAT_RECEIVE_MESSAGE:
      return {...state, chat: concat(state.chat, assign({}, action.payload))};
