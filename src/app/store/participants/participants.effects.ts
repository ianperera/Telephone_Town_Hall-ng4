...
import { ParticipantStatus } from '../../models/events';
import { DeselectParticipant } from '../ui/ui.actions';
...
export class ParticipantEffects {
  ...
  @Effect() // Deselect participant when participant disconnects.
  deselectParticipant$: Observable<Action> = this.actions$
    .ofType(p.UPDATE)
    .map((a: p.Update) => a.payload)
    .withLatestFrom(this.store.select(fromRoot.getSelectedParticipant))
    .filter(([p, s]) => p.status === ParticipantStatus.disconnected && p.id == s.id)
    .map(() => new DeselectParticipant());
